...
input=[SEAtrainingdata;SEAtestingdata];
outputclass=[SEAtrainingclass;SEAtestingclass];
normalized_input=normal_class_zero_mean(input);
[nData,nData1]=size(input);
[creditcardoutput,pendigits_Data]=modify_dataset_zero_class(outputclass);
chunk_size=floor(nData/nFolds);
data=[normalized_input creditcardoutput];
...
for  k=1:chunk_size:nData
    ...
    lambdaD=min(1-exp(-counter/nFolds),0.08);
